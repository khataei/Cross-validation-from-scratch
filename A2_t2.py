...
def main():
    ...
    df = read_csv(observation_file, sep="\t", header=None)
...
#                print(f,neigborNumber)
                ...
                scoreMatrix.iloc[f - 1,
                                 neigborNumber - 1] = scoreMatrix.iloc[f - 1,
...
def SplitFold(totalDF, TotalfoldNumber):
    ...
    for i in range(
            sizeA):  # asigning almost equal number of class A and B to folds
        folds[i % TotalfoldNumber, i // TotalfoldNumber, :] = cA[i, :]

    # we start from sizeA to continue with the next fold, not necceserily the
    # first one
    for i in range(sizeA, rows, 1):
...
def Feature_Select_Corr(featureDataSet):
    ...
    for j in range(P - 1):
        for i in range(
                j + 1, P):  # moving in the upper triangle of the correlation matrix
            if (abs(cordf.iloc[j, i]) > 0.5):
                # ignoring the second one by setting to NaN
